   ...
   <div class="space-y-6">
      ...
      <div class="flex items-start justify-between gap-4 pr-2">
         <div class="flex-1">
            ...
            <p class="text-amber-200">Gerencie os schemas de credenciais disponíveis</p>
         </div>
         <div class="flex-shrink-0">
            <Button @click="openCreateSheet" class="bg-amber-600 hover:bg-amber-700 cursor-pointer text-white">
      ...
      <div class="space-y-4 max-h-[60vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-amber-600 scrollbar-track-white/10">
         <div v-if="!appStore.isLoading">
               ...
               <div class="flex items-center justify-between">
                  <div class="flex items-center space-x-4 flex-1">
                     ...
                     <div class="flex-1">
                        ...
                        <div class="text-sm">
                           <div class="mb-1">
                              <span class="text-amber-300">Schema ID: </span>
                  ...
                  <div class="flex items-center space-x-2">
                     <Button
                        ...
                        variant="ghost"
                        size="sm"
                        class="text-amber-200 cursor-pointer hover:text-white hover:bg-white/10"
                        title="Ver Detalhes"
                     >
                        <Eye class="w-4 h-4" />
                     </Button>
            ...
            <div v-if="filteredCredentials.length === 0" class="text-center py-12">
               <CreditCard class="w-16 h-16 text-amber-600 mx-auto mb-4" />
               ...
               <Button v-if="!searchTerm && (!versionFilter || versionFilter === 'all')" @click="openCreateSheet" class="bg-amber-600 hover:bg-amber-700 text-white cursor-pointer">
                  <Plus class="w-4 h-4 mr-2" />
      ...
      >
         <div
            class="bg-gradient-to-br from-amber-900/90 to-amber-950/90 backdrop-blur-md rounded-xl border border-white/20 p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto"
            @click.stop
         >
            ...
            <div class="space-y-4">
               <div class="bg-white/5 rounded-lg p-4">
                  <label class="text-sm font-medium text-amber-300">Nome</label>
                  <p class="text-lg text-white font-semibold mt-1">{{ credentialStore.currentCredential.name }}</p>
               </div>

               <div class="bg-white/5 rounded-lg p-4">
                  <label class="text-sm font-medium text-amber-300">Versão</label>
                  <p class="text-lg text-white font-semibold mt-1">{{ credentialStore.currentCredential.version }}</p>
               </div>

               <div class="bg-white/5 rounded-lg p-4">
                  <label class="text-sm font-medium text-amber-300">Schema ID</label>
                  <p class="text-sm text-white font-mono break-all mt-1">{{ credentialStore.currentCredential.id }}</p>
               </div>

               <div class="bg-white/5 rounded-lg p-4">
                  <label class="text-sm font-medium text-amber-300 mb-2 block">Atributos</label>
                  <div class="flex flex-wrap gap-2">
                     <span
                        v-for="attr in credentialStore.currentCredential.attributes"
                        :key="attr"
                        class="px-3 py-1 bg-amber-600/30 text-amber-100 rounded-full text-sm font-medium border border-amber-400/30"
                     >
                        {{ attr }}
                     </span>
                  </div>
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t border-white/20 mt-6">
               <Button @click="closeDetailsModal" class="bg-amber-600 cursor-pointer hover:bg-amber-700 text-white">
                  Fechar
               </Button>
            </div>
         </div>
      ...
      <Sheet :open="isSheetOpen" @update:open="closeSheet">
         <SheetContent class="bg-gradient-to-br px-4 from-amber-900 via-orange-800 to-yellow-900 border-l border-white/20 text-white overflow-y-auto">
            <SheetHeader>
               <SheetTitle class="text-white text-2xl">
                  {{ isEditMode ? 'Editar Credencial' : 'Nova Credencial' }}
               </SheetTitle>
               <SheetDescription class="text-amber-200">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-6 mt-6">
               ...
               <div class="space-y-3">
                  <div class="flex justify-between items-center">
                     ...
                     <Button
                        ...
                        size="sm"
                        variant="ghost"
                        class="text-amber-200 hover:text-white cursor-pointer hover:bg-white/10"
                     >
                  ...
                  >
                     <Input
                        ...
                        class="flex-1 bg-white/20 border-white/30 text-white placeholder:text-amber-200"
                     />
                     <Button
                        type="button"
                  ...
                  <p v-if="credentialForm.attributes.length == 1 && !credentialForm.attributes[0]" class="text-xs text-amber-300">
               ...
               <SheetFooter class="flex pt-6 border-t border-white/20">
                <Button
                     ...
                     class="bg-amber-600 hover:bg-amber-700 cursor-pointer text-white"
                  >
                  ...
                  <Button
                     ...
                     @click="closeSheet"
                     variant="ghost"
                     class="text-amber-200 cursor-pointer"
                  >
                     Cancelar
                  </Button>
               </SheetFooter>
