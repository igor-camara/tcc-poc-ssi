...
class User:
    ...
    def __init__(self, email=None, password=None, first_name=None, last_name=None, 
                 id=None, did=None, verkey=None, did_metadata=None, 
        ...
        self.did = did
        self.verkey = verkey
        self.did_metadata = did_metadata or {}
    ...
    def _from_row(cls, row):
        ...
        user = cls(
            ...
            did=row['did'],
            verkey=row['verkey'],
            did_metadata=json.loads(row['did_metadata']) if row['did_metadata'] else {},
            ...
            if existing:
                ...
                conn.execute('''
                    UPDATE users SET 
                        ...
                        is_active = ?, did = ?, verkey = ?, did_metadata = ?, updated_at = ?
                    WHERE id = ?
                ''', (
                    self.email, self.password_hash, self.first_name, self.last_name,
                    int(self.is_active), self.did, self.verkey, 
                    json.dumps(self.did_metadata), self.updated_at.isoformat(), self.id
                ))
            else:
                # Insert
                conn.execute('''
                    ...
                    (id, email, password_hash, first_name, last_name, is_active, 
                     did, verkey, did_metadata, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    self.id, self.email, self.password_hash, self.first_name, self.last_name,
                    int(self.is_active), self.did, self.verkey, json.dumps(self.did_metadata),
